...
def create_OP_CHECKMULTISIG_signature(txin, txin_scriptPubKey, txout_script_1, txout_script_2                                      ,seckey):
    tx = CMutableTransaction([txin], [txout_script_1,txout_script_2])
    sighash = SignatureHash(CScript(txin_scriptPubKey), tx,
                            0, SIGHASH_ALL)
    sig = seckey.sign(sighash) + bytes([SIGHASH_ALL])
    return sig
...
def create_signed_transaction_2_outputs(txin, txout_1,txout_2, txin_scriptPubKey,
                                                txin_scriptSig):
